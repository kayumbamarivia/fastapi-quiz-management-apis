...
async def create_question(q: QuestionBase, db: db_dependency):
...
async def get_question_by_id(q_id: int, db: db_dependency):
...
async def update_question_by_id(q_id: int, q: QuestionBase, db: db_dependency):
    db_question = db.query(Questions).filter(Questions.id==q_id).first()
...
async def delete_question_by_id(q_id:int, db: db_dependency):
    db_question=db.query(Questions).filter(Questions.id==q_id).first()
    if not db_question:
        raise HTTPException(status_code=404, detail=NOT_FOUND)
...
async def delete_all_questions(db: db_dependency):
    try:
